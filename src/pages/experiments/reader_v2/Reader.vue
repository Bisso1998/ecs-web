            ...
            <nav id="sidebar" v-if="getPratilipiLoadingState === 'LOADING_SUCCESS'">
                ...
                <div class="book-info">
                    ...
                    <div class="follow-btn-w-count" v-if="!getAuthorData.following"><!-- Follow Button -->
                        ...
                        </button><span><b>{{ getAuthorData.followCount | showThousandsInK }}</b></span>
                    </div>
                    <div class="follow-btn-w-count" v-else><!-- Following Button -->
                        <button @click="checkLoginStatusAndFollowOrUnfollowAuthor"><i class="material-icons">check</i> __("author_following")</button><span><b>{{ getAuthorData.followCount | showThousandsInK }}</b></span>
                    </div>
                </div>
...
import Reviews from '@/components/experiments/reader_v2/Reviews.vue';
import ControlReviews from '@/components/Reviews.vue';
import Recommendation from '@/components/Recommendation.vue';
import OpenInApp from '@/components/OpenInApp.vue';
import ShareStrip from '@/components/experiments/reader_v2/ShareStrip.vue';
...
export default {
    ...
    methods: {
        ...
        checkLoginStatusAndFollowOrUnfollowAuthor() {
            ...
            this.triggerAnanlyticsEvent(`${action}_INDEX_READER`, 'WGEN006', {
                ...pratilipiAnalyticsData,
                'USER_ID': this.getUserDetails.userId,
    ...
    watch: {
        ...
        'getPratilipiLoadingState'(status) {
            if (status === 'LOADING_SUCCESS') {
                const pratilipiAnalyticsData = this.getPratilipiAnalyticsData(this.getPratilipiData);
                this.triggerAnanlyticsEvent('LANDED_READERM_READER', 'WGEN006', {
                    ...pratilipiAnalyticsData,
                    'USER_ID': this.getUserDetails.userId
                });
            }
